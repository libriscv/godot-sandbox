#!/bin/bash
# Benchmark suite. The JIT and the interpreter need separate Godot runs, because
# libriscv caches the execution mode per binary for the lifetime of the process.
#
# A third mode -- a native translation baked ahead of time -- is not measured
# here: nothing in this addon produces one. test_bintr.gd covers the loader
# against a fixture object.
#
...
for mode in jit nojit; do
	...
	# Only numbered measurement archives belong to this mode's repeat set.
