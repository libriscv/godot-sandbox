...
Variant ResourceFormatLoaderZig::GODOT_CPP_FUNC (load)(const String &p_path, const String &original_path, bool use_sub_threads, int32_t cache_mode) const {
	Ref<ZigScript> model = memnew(ZigScript);
	model->_set_source_code(FileAccess::get_file_as_string(p_path));
	return model;
}
PackedStringArray ResourceFormatLoaderZig::GODOT_CPP_FUNC (get_recognized_extensions)() const {
	PackedStringArray array;
	array.push_back("zig");
	return array;
}
bool ResourceFormatLoaderZig::GODOT_CPP_FUNC (handles_type)(const StringName &type) const {
	String type_str = type;
	return type_str == "ZigScript" || type_str == "Script";
}
String ResourceFormatLoaderZig::GODOT_CPP_FUNC (get_resource_type)(const String &p_path) const {
