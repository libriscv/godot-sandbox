...
Variant ResourceFormatLoaderRust::GODOT_CPP_FUNC (load)(const String &p_path, const String &original_path, bool use_sub_threads, int32_t cache_mode) const {
	Ref<RustScript> model = memnew(RustScript);
	model->_set_source_code(FileAccess::get_file_as_string(p_path));
	return model;
}
PackedStringArray ResourceFormatLoaderRust::GODOT_CPP_FUNC (get_recognized_extensions)() const {
	PackedStringArray array;
	array.push_back("rs");
	return array;
}
bool ResourceFormatLoaderRust::GODOT_CPP_FUNC (handles_type)(const StringName &type) const {
	String type_str = type;
	return type_str == "RustScript" || type_str == "Script";
}
String ResourceFormatLoaderRust::GODOT_CPP_FUNC (get_resource_type)(const String &p_path) const {
